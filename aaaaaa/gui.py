...
from PyQt5.QtGui import QPixmap, QPainter, QBitmap, QIcon, QFont, QCursor, QDesktopServices
...
class MainWindow(QMainWindow):
    ...
    def makebuttons(self):
        with open(rf'{scuffedpath[0]}\valock\config.json') as file:
        ...
        for agent in agents:
            ...
            if x > 280:
                ...
                y += 50
        button = QPushButton(self)
        button_icon = QPixmap(rf'assets\github.png')
        button.setIcon(QIcon(button_icon))
        button.setIconSize(button_icon.size())
        button.setGeometry(x-5, y-10, button_icon.width(), button_icon.height())
        button.setStyleSheet("background-color: transparent; border: none;")
        button.clicked.connect(self.connecturl)
    def connecturl(self):
        url = QUrl("https://github.com/pythonmaster9000")
    ...
    def selectbutton(self, checked, button):
       # print('angry word2')
        ...
        if checked:
            ...
            with open(rf'{scuffedpath[0]}\valock\config.json', 'w') as outfile:
...
def rungui(agent, pathenv='opt'):
   # print('angry word0')
    scuffedpath[0] = pathenv
